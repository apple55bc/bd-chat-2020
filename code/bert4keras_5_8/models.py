...
import numpy as np
from bert4keras_5_8.layers import *
import json
import warnings
...
class Transformer(object):
    ...
    def __init__(
            ...
            input_is_emb=False,  # 使用了之后，则输入权重和output的vocab不共享，且重新初始化input权重
    ):
    ...
    def load_weights_from_checkpoint(self, checkpoint, mapping=None):
        ...
        if len(not_found) > 0:
            print('Not found layer: {}'.format(not_found))
...
class BERT(Transformer):
    ...
    def prepare_inputs(self):
        ...
        """
        x_in = Input(shape=(None, self.embedding_size) if self.input_is_emb else (None,), name='Input-Token')
    ...
    def prepare_embeddings(self, inputs):
        ...
        self._embedding = Embedding(input_dim=self.vocab_size,
    ...
    def prepare_final_layers(self, inputs):
        ...
        if self.with_mlm:
            ...
            embedding_dense = EmbeddingDense(embedding_name='Embedding-Token',
                                             activation=mlm_activation,
                                             name='MLM-Proba')  # <<<
            if not self._embedding.built:  # <<<
                self._embedding.build(input_shape=(None, None))  # <<<
            embedding_dense.kernel = K.transpose(self._embedding.embeddings)  # <<<
            x = embedding_dense(x)  # <<<
            outputs.append(x)
...
def build_transformer_model(config_path,
    ...
    if 'vocab_size' in kwargs.keys() and keep_tokens is None:  # <<<
        keep_tokens = list(range(kwargs['vocab_size']))  # <<<
...
if __name__ == '__main__':
    _m = build_transformer_model(config_path=r'D:\project\my_kidx\generativedialogue\data\bert/bert_config.json',
                                 # input_is_emb=66,
                                 checkpoint_path=r'D:\project\my_kidx\generativedialogue\data\bert/bert_model.ckpt',
                                 # checkpoint_path=None,
                                 with_mlm=True,
                                 num_hidden_layers=1,
                                 input_is_emb=None, )
